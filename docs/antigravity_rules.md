...
- **Health Check First**: Run `.\.conda\python.exe src/utils/check_health.py` to ensure the environment and data links are valid.
...
- Update `STATUS.md` after every major milestone by checking off completed tasks.
...
- **Log it**: Make sure the change is clearly documented in the git commit message and referenced in `STATUS.md` if applicable.
...
- **Verification**: If an AI produces a script, it must be run and its output validated before marking the task complete in `STATUS.md`.
